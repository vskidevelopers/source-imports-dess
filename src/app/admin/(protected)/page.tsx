...
const STATUSES = ['received', 'sourcing', 'confirmed', 'shipped', 'in_transit', 'delivered', 'closed']
...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        console.log('[AdminDashboard] Mounted | Fetching quotes')
        const fetchQuotes = async () => {
            ...
            else console.log(`[AdminDashboard] ✅ Loaded ${data?.length || 0} quotes`)
    ...
    const handleStatusUpdate = async (quoteId: string, newStatus: string) => {
        setUpdating(quoteId)
        console.log(`[AdminDashboard] Updating ${quoteId} → ${newStatus}`)
        try {
            const { error } = await supabase.from('quotes').update({ status: newStatus }).eq('quote_id', quoteId)
            if (error) throw error

            setQuotes(prev => prev.map(q => q.quote_id === quoteId ? { ...q, status: newStatus } : q))
            console.log('[AdminDashboard] ✅ Status updated | Optimistic UI applied')
        } catch (err) {
    ...
    const handleExportCSV = () => {
        console.log('[AdminDashboard] CSV export triggered')
        const headers = ['Quote ID', 'Name', 'WhatsApp', 'Category', 'Item', 'Qty', 'Status', 'Date']
        const rows = quotes.map(q => [
            q.quote_id, q.customer_name, q.whatsapp, q.category,
            `"${q.item_description.replace(/"/g, '""')}"`, q.quantity, q.status,
        ...
        a.click()
        console.log('[AdminDashboard] ✅ CSV downloaded')
    }
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    <input
                        type="text"
                        placeholder="Search ID, name, or WA..."
                        value={search}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody>
                            {filtered.map((q) => (
                                <tr key={q.id} className="border-b hover:bg-gray-50 transition">
                                    ...
                                    <td className="p-4 hidden md:table-cell">
                                        <div className="truncate max-w-[200px]" title={q.item_description}>{q.item_description}</div>
                                        <div className="text-xs text-gray-500">{q.quantity}x • {q.category}</div>
                                    </td>
                                    <td className="p-4">
                                        <select
                                            value={q.status}
                                            onChange={(e) => handleStatusUpdate(q.quote_id, e.target.value)}
                                            disabled={updating === q.quote_id}
                                            className={`px-2 py-1 border rounded text-xs bg-white focus:ring-2 focus:ring-blue-500 outline-none ${STATUS_COLORS[q.status] || ''}`}
                                        >
                                            {STATUSES.map(s => <option key={s} value={s}>{s.replace('_', ' ').toUpperCase()}</option>)}
                                        </select>
                                    </td>
                                    <td className="p-4 text-gray-500 whitespace-nowrap">{new Date(q.created_at).toLocaleDateString()}</td>
                                    <td className="p-4">
                                        <button
                            ...
                            {filtered.length === 0 && (
                                <tr><td colSpan={6} className="p-8 text-center text-gray-500">No quotes match your search</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
