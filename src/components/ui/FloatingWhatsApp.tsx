...
export function FloatingWhatsApp() {
    ...
    useEffect(() => {
        console.log('[FloatingWhatsApp] Mounted | Number:', process.env.NEXT_PUBLIC_WHATSAPP_NUMBER)
    ...
    const handleClick = () => {
        console.log('[FloatingWhatsApp] Button clicked | Opening WhatsApp')
    ...
    return (
        <button
            onClick={handleClick}
            className={`fixed bottom-6 right-6 z-50 bg-[#25D366] hover:bg-[#128C7E] text-white p-4 rounded-full shadow-lg transition-all duration-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'
                }`}
